...
const D3D11_INPUT_ELEMENT_DESC ParticleSystem::ParticleElementDesc[ PARTICLE_ELEMENT_DESC_SIZE ] = 
{
	...
	{"SPEED",    0, DXGI_FORMAT_R32_FLOAT,       0, 44, D3D11_INPUT_PER_VERTEX_DATA, 0},
};
...
: mInitVB(0), mDrawVB(0), mStreamOutVB(0), mTexArraySRV(0), mRandomTexSRV(0)
{
	...
	ID3D11Device* pD3DDevice = Application::s_pApp->GetDevice();

	ParticleInputLayout = 0;
	...
	pD3DDevice->CreateInputLayout(ParticleElementDesc, PARTICLE_ELEMENT_DESC_SIZE, passDesc.pIAInputSignature, passDesc.IAInputSignatureSize, &ParticleInputLayout);
...
void ParticleSystem::Draw(ID3D11DeviceContext* dc, XMMATRIX& ViewProj)
{
	//
	// Set constants.
	//
